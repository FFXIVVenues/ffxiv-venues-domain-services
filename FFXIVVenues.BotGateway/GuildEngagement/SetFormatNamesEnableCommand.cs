...
[DiscordCommand("server formatnames enable", "Set Veni to format the display names of Venue Managers in this discord server.", GuildPermission.ManageRoles, InteractionContextType.Guild)]
public class SetFormatNamesEnableCommand(IRepository repository) : ICommandHandler
{
    public async Task HandleAsync(SlashCommandVeniInteractionContext slashCommand)
    {
        ...
        guildSettings.FormatNames = true;
        var upsertTask = repository.UpsertAsync(guildSettings);

        await slashCommand.Interaction.RespondAsync($"Oki! I'll set their names for them! 🥰", ephemeral: true);
        await upsertTask;
    }
